...
import { ArrowLeft, ArrowUpDown, Info, Settings } from "lucide-react"
import { useRouter } from "next/navigation"
import { parseUnits, formatUnits } from "ethers"
import swapAbi from "../abis/swap.json"
import erc20Abi from "../abis/erc20.json"
import { SWAP_ADDRESS, TOKEN_LIST } from "../constants/addresses"
...
const TokenSwap = () => {
  ...
  const [tokens, setTokens] = useState(TOKEN_LIST)

  const provider = new JsonRpcProvider("https://sepolia.infura.io/v3/30a821291d5f47b4bbb8c23c05ba000f");
  ...
  const updateToAmount = (value: string) => {
    if (!value || isNaN(Number(value))) return setToAmount("")
  ...
  const estimateRate = async () => {
    try {
      ...
      const swapContract = new ethers.Contract(SWAP_ADDRESS, swapAbi, provider)

      const fromInfo = getTokenInfo(fromToken)
      const toInfo = getTokenInfo(toToken)
      if (!fromInfo || !toInfo || !fromAmount) return

      const amountIn = parseUnits(fromAmount, fromInfo.decimals)
      const amountsOut = await swapContract.getAmountsOut(amountIn, [fromInfo.address, toInfo.address])
      const amountOut = formatUnits(amountsOut[1], toInfo.decimals)

      setExchangeRate(parseFloat(amountOut) / parseFloat(fromAmount))
  ...
  const handleSwap = async () => {
    try {
      ...
      if (fromToken !== "ETH") {
        const fromTokenContract = new ethers.Contract(fromInfo.address, erc20Abi, signer)
        const allowance = await fromTokenContract.allowance(await signer.getAddress(), SWAP_ADDRESS)
        if (allowance.lt(amountIn)) {
          const approveTx = await fromTokenContract.approve(SWAP_ADDRESS, ethers.MaxUint256)
